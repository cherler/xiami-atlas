...
- **elevenlabs-changelog**（3 条）：发布 eleven-v3-ga、发布 eleven-v3-conv、发布 eleven-music-v2
...
- `recraft-endpoints` recraft × i-outpaint —— 锚「The area to be generated can be specified either by per-side pixel mar」
- `bfl-flux2-multiref` flux × i-styleref —— 锚「Apply the colors, patterns, and surface tones of the animal in Image 2」
- `bfl-flux2-overview` flux × i-textrender —— 锚「Specialized for typography. Best for text rendering and preserving sma」
...
- `meshy-docs-api` meshy × d-t23d —— 锚「Create a Preview Task」
- `meshy-docs-api` meshy × d-multiview —— 锚「Batch Images to 3D」
- `rodin-site` rodin × d-t23d —— 锚「From text prompt to 3D object」
- `rodin-site` rodin × d-i23d —— 锚「Upload a picture, get a 3D object instantly」
- `rodin-api-docs` rodin × d-multiview —— 锚「up to 5 images. As the form data request will preserve the order of th」
- `rodin-api-docs` rodin × d-pbr —— 锚「PBR: Physically Based Materials, including base color texture, metalli」
- `rodin-api-docs` rodin × d-edit —— 锚「split a Rodin-generated Asset into multiple submodels」
- `rodin-api-docs` rodin × d-format —— 锚「Possible values are glb, usdz, fbx, obj, and stl. Default is glb」
- `stability-spar3d` spar3d × d-edit —— 锚「Unprecedented Control: Allows users to directly edit the point cloud b」
- `heygen-docs` heygen × p-video —— 锚「from footage, images, or a text description」
- `hf-hallo2` hallo2 × p-emotion —— 锚「pose_weight --face_weight --lip_weight --face_expand_ratio」
- `nvidia-groot-gh` groot × e-openweights —— 锚「This model is released under the NVIDIA Open Model License Agreement」
- `nvidia-groot-gh` groot × e-finetune —— 锚「Developers and researchers can post-train GR00T N1.7 with real or synt」
- `openvla-gh` openvla × e-openweights —— 锚「All OpenVLA checkpoints, as well as our training codebase are released」
...
- `sci-wn-gh` 模型 weathernext 版本 —— 锚「the global, medium-range atmospheric and cyclone forecasting model dev」
...
- `wiki-midjourney` 发布 midjourney-V6.1-画质与细节一致性迭代 —— 锚「V6.1 ... July 31, 2024」
- `wiki-midjourney` 发布 midjourney-V7-Draft Mode 与 Omni Reference —— 锚「V7 ... April 4, 2025」
- `krea-blog` 发布 krea-img-Krea 2-第一个完全从零训练的自研基座 —— 锚「our first foundation image model built completely from scratch」
- `google-nb-blog` 发布 gemini-image-Nano Banana Pro-接上自家新一代大模型的推理，4K 与复杂版面 —— 锚「our new state-of-the art image generation and editing model.」
- `google-nb-blog` 发布 gemini-image-Nano Banana 2-Pro 的质量按 Flash 的速度和价钱出 —— 锚「our latest state-of-the-art image model.」
- `recraft-blog` 发布 recraft-Recraft V4-按设计审美重训的一代 —— 锚「brings true visual taste to AI image generation」
- `recraft-blog` 发布 recraft-Recraft V4.1-写实更自然、短提示词也能懂 —— 锚「Give V4.1 just a few words, and it'll give you something worth keeping」
- `mj-updates` 发布 midjourney-V8 Alpha-出图快约 5 倍 + 原生 2K（--hd） —— 锚「V8 is an incredible model. It's much better at following detailed dire」
- `comfy-docs` 发布 hidream-HiDream-I1-17B 出图底座，MIT —— 锚「HiDream-I1 is a text-to-image model officially open-sourced by HiDream」
- `comfy-docs` 发布 ernie-image-ERNIE-Image-8B DiT 正式档，apache-2.0 —— 锚「ERNIE-Image is an open text-to-image model by Baidu, licensed under Ap」
- `comfy-docs` 发布 ovis-image-Ovis-Image-7B-7B 专攻图上出字 —— 锚「Ovis-Image is a 7B text-to-image model built upon Ovis-U1, specificall」
- `comfy-docs` 发布 lens-Lens-双流 MMDiT + FLUX.2 的 VAE —— 锚「Lens is an open text-to-image model by Microsoft, licensed under MIT. 」
- `comfy-docs` 发布 mage-flow-Mage-Flow-4B 出图与指令改图一体 —— 锚「Mage-Flow is a compact 4B-scale generative stack from Microsoft Resear」
...
- `meshy-changelog` 发布 meshy-format —— 锚「Added optional target_formats parameter ... Specify which 3D formats t」
- `meshy-changelog` 发布 meshy-7 —— 锚「Added ai_model: meshy-7 to the Image to 3D API: our newest 3D generati」
- `heygen-changelog` 发布 heygen-v3api —— 锚「New Voice Creation Endpoint」
- `heygen-changelog` 发布 heygen-avatar3 —— 锚「videos to include support for the Avatar III engine」
- `heygen-changelog` 发布 heygen-avatar5 —— 锚「Per-second video generation rates for Avatar V now match Avatar IV」
- `heygen-changelog` 发布 heygen-batch —— 锚「Batch APIs for Translations, Lipsyncs, and Assets」
- `heygen-changelog` 发布 heygen-consent —— 锚「Consent Video Upload Available for All Enterprise Accounts」
- `elevenlabs-changelog` 发布 eleven-v3-ga —— 锚「v3 is out of alpha - it's more stable, accurate and has lower latency」
- `elevenlabs-changelog` 发布 eleven-v3-conv —— 锚「Eleven v3 conversational model: Added eleven_v3_conversational to the 」
- `elevenlabs-changelog` 发布 eleven-music-v2 —— 锚「Music v2 introduces chunk-based composition plans built from Generatio」
- `openvla-gh` 发布 openvla-7b —— 锚「trained on 970K robot manipulation episodes from the Open X-Embodiment」
- `pi-openpi-gh` 发布 pi0-fast —— 锚「FAST model for fine-tuning」
- `molmoact-card` 发布 molmoact-2 —— 锚「builds on Molmo2-ER, an embodied-reasoning VLM backbone, and connects 」
...
- `hf-lineage` cosyvoice × s-long —— 锚「Bi-Streaming: Support both text-in streaming and audio-out streaming, 」
...
- `luma-ray32` 模型 luma-ray 版本
- `meta-muse` 模型 muse-spark 版本
- `lux3d-site` 模型 lux3d 版本
- `hf-locateanything` 模型 locateanything 版本
- `wan3-qbitai` 模型 wan 版本
- `modelscope-orgs` 模型 internvla 版本
